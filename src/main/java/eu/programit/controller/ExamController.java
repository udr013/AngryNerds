...
import eu.programit.service.ExamService;
...
public class ExamController {
	...
	@Autowired
	private ITestViewsService iTestViewsService;
	...
	String createExam(@ModelAttribute("testviews") TestViews testViews, Model model,
			ExamQuestionSelectedForm examQuestionSelectedForm) {
		...
		// ExamQuestionSelectedForm examQuestionSelectedForm = new
		// ExamQuestionSelectedForm();
		try {
		...
		this.examService.save(emptyExam);
		System.out.println("The id from the created exam is " + emptyExam.getId());

		this.exam = emptyExam; // do not use instance vars in controllers

		// this.myTestView.setId((int) emptyExam.getId()); // rloman consider
		// refactor all int to long
		...
		// model.addAttribute("mySelectedQuestions",
		// mySelectedQuestions.getTestViewsContent().get(q.getQuestionID()));
		System.out.println("this is createExam" + testViews);
	...
	public String saveQuestToExam(@ModelAttribute ExamQuestionSelectedForm examQuestionSelectedForm) {
		...
		if (answ != null) {
			for (int s : answ) {
				...
				// mySelectedQuestions.setSelectedQuestion(new
				// Integer(myTestView.getId()), s.g);
		...
		try {
			...
			// mySelectedQuestions.setSelectedQuestions((Map<Integer,
			// List<Integer>>) answ);
		...
		// refactor dit naar het vinden van de id naar aanleiding van de
		// binnengekomen parameters
		Exam exam = examService.findById((int) this.exam.getId());

		for (int i : answ) {
			Question question = this.iQuestionService.findById(i);
			exam.addQuestion(question);
			System.out.println("Het wordt opgebouwd: "+exam);
		}
		
		System.out.println("Het examen bestaat uit: "+exam);

		this.examService.save(exam);
		// mySelectedQuestions.setTestViewsContent(new
		// Integer(myTestView.getId()),
